...
```sh
go get -u 'github.com/stainless-sdks/knock-mapi-go@v0.0.1-alpha.0'
```
...
Optional primitive types are wrapped in a `param.Opt[T]`. Use the provided constructors set `param.Opt[T]` fields such as `knockmapi.String(string)`, `knockmapi.Int(int64)`, etc.
...
```go
params := knockmapi.ExampleParams{
	ID:          "id_xxx",                // required property
	Name:        knockmapi.String("..."), // optional property
	Description: param.NullOpt[string](), // explicit null property

	Point: knockmapi.Point{
		X: 0,                // required field will serialize as 0
		Y: knockmapi.Int(1), // optional field will serialize as 1
		// ... omitted non-required fields will not be serialized
	},

	Origin: knockmapi.Origin{}, // the zero value of [Origin] is considered omitted
}
...
custom := param.OverrideObj[knockmapi.FooParams](12)
```
...
client.Workflows.List(context.TODO(), ...,
	// Override the header
...
`*knockmapi.Error`. This contains the `StatusCode`, `*http.Request`, and
...
if err != nil {
	...
	panic(err.Error()) // GET "/v1/workflows": 400 Bad Request { ... }
}
```
...
We also provide a helper `knockmapi.File(reader io.Reader, filename string, contentType string)`
...
params := FooNewParams{
    ...
    Data: FooNewParamsData{
        FirstName: knockmapi.String("John"),
    },
}
